...
export class AnimationManager<T> {
  ...
  animate<A>(
    ...
    if (this.repeatStatus[0]) {
      // 在重复模式中，将动画保存到重复区间
      this.repeatStatus[3].push({ context, animation: Object.assign({}, animation), duration, by });
    }
    this.animations.push({ context, animation: Object.assign({}, animation), duration, by });
    return this;
  }
...
export function registerAnimation<T>(
  ...
  if (widget) {
    ...
    const elapsed: WatchSource<number> = () => widgetInstance && widgetInstance.elapsed ? widgetInstance.elapsed : player.elapsed
